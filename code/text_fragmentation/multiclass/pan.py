...
def extract_annotations_from_files(path, tagname):
    ...
    if not os.path.exists(path):
        logging.debug("Path not accessible:", path)
...
def main(micro_averaged, plag_path, plag_tag_name, det_path, det_tag_name):
    ...
    plag_score = plagdet_score(rec, prec, gran)
    logging.debug('Plagdet Score', plag_score)
    logging.debug('Recall', rec)
    logging.debug('Precision', prec)
    logging.debug('Granularity', gran)

    stats = dict()
    stats['precision'] = prec
    stats['recall'] = rec
    stats['granularity'] = gran
    stats['plag score'] = plag_score
