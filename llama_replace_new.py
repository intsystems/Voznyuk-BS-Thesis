...
with open('2_paragraphs.txt', 'a') as documents_with_2_gen_paragraphs,\
        open('3_paragraphs.txt', 'a') as documents_with_3_gen_paragraphs,\
            open('4_paragraphs.txt', 'a') as documents_with_4_gen_paragraphs:
    for number, article in enumerate(tqdm(dataset['0'][start:12000])):
        list_of_paragraphs = preprocess(article)
        ...
        if cnt_of_paragraphs >= 6 and number_of_4_paragraph_articles < 2000:
            ...
            documents_with_4_gen_paragraphs.write(str(number))
            documents_with_4_gen_paragraphs.write("\n")

        elif cnt_of_paragraphs >= 5 and number_of_3_paragraph_articles < 3500:
            is_success = process_article(article, number, 3)
            if not is_success:
                continue
            number_of_3_paragraph_articles += 1

            documents_with_3_gen_paragraphs.write(str(number))
            documents_with_3_gen_paragraphs.write("\n")

        elif cnt_of_paragraphs >= 4 and number_of_2_paragraph_articles < 4500:
            is_success = process_article(article, number, 2)
            if not is_success:
                continue
            number_of_2_paragraph_articles += 1

            documents_with_2_gen_paragraphs.write(str(number))
            documents_with_2_gen_paragraphs.write("\n")
        else:
            continue
